...
def build_runner(args, model: LanguageModel):
    ...
    if model.model_style == LMStyle.LocalAPI:
        from lcb_runner.runner.localapi_runner import LocalAPIRunner
        if "DeepSeek-Coder-V2-Lite-Instruct" in model.model_name:
            model.model_style = LMStyle.DeepSeekAPI
        if "DeepSeek-R1-Distill-Qwen-14B" in model.model_name:
            model.model_style = LMStyle.DeepSeekAPI
        if "deepseek-v3-ep" in model.model_name:
            model.model_style = LMStyle.DeepSeekAPI
        if "gpt-5" in model.model_name:
            model.model_style = LMStyle.DeepSeekAPI
        if "claude-sonnet-4-20250514" in model.model_name:
            model.model_style = LMStyle.DeepSeekAPI
        if "deepseek-v3" in model.model_name:
            model.model_style = LMStyle.DeepSeekAPI
