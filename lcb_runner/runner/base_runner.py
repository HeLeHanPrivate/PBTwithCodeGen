...
from lcb_runner.runner.scenario_router import Scenario, TestCaseForRepair


def _metadata_with_problem_interface(metadata, problem, platform):
...
class BaseRunner(ABC):
    ...
    def run_main_repair(self, benchmark: list, format_prompt: callable) -> list[list[str]]:
        ...
        repairbase_path = getattr(self.args, "repairbase_path", "") or ""
        if not repairbase_path:
            exdirname = ""
            ...
            if self.args.repairbase == Scenario.selfrepair:
                ...
                testcase = ""
            repairbase_temperature = getattr(self.args, "repairbase_temperature", None)
            if repairbase_temperature is None:
                repairbase_temperature = self.args.temperature
            repairbase_path = (
                f"output/{self.model.model_repr}{exdirname}/"
                f"{self.args.repairbase}{testcase}_{self.args.codegen_n}_{repairbase_temperature}_eval_all.json"
            )
        else:
            repairbase_path = str(repairbase_path)
        with open(repairbase_path) as f:
            check_metadata_list = json.load(f)

        if self.args.scenario == Scenario.checkerextend:
            from lcb_runner.runner.our_method import MyPipeline
            mypipeline = MyPipeline(self.args)
            outputs = mypipeline.our_method_pipeline(benchmark, self.model.model_style, self.args, check_metadata_list, self.prompts_to_outputs)
            return outputs
                ...
                if problem.question_id == check_metadata['question_id']:
                    ...
                    for code_idx in range(len(code_list)):
                        if self.args.scenario == Scenario.selfrepair:
                            prompt_metadata = _metadata_with_problem_interface(
                                metadata[code_idx],
                                problem,
                                platform,
                            )
                            ...
                            if self.args.testcaseforrepair == TestCaseForRepair.publiccase and public_graded_list[code_idx]:
                                prompt = ""
                            if prompt == "":
                                outputs[problem_idx][code_idx] = output_list[code_idx]
                                continue
                        elif self.args.scenario == Scenario.testcasegeneration:
                            prompt = format_prompt(
                                question_content,
                                self.model.model_style,
                                code_list[code_idx],
                            ...
                            if prompt == "":
                                ...
                                continue
                        elif self.args.scenario == Scenario.checkerextend:
                            prompt = format_prompt(
                                question_content,
                                self.model.model_style,
                                code_list[code_idx],
    ...
    def run_main(self, benchmark: list, format_prompt: callable) -> list[list[str]]:
        if self.args.scenario == Scenario.selfrepair or self.args.scenario == Scenario.testcasegeneration or self.args.scenario == Scenario.checkerextend:
        ...
        if self.args.scenario == Scenario.codegeneration and not getattr(self.args, "no_retry_empty_code", False):
            from lcb_runner.prompts.code_generation import format_prompt_generation_code_only
            from lcb_runner.utils.extraction_utils import extract_code

            retry_prompts = []
            retry_indices = []
            for idx, output_list in enumerate(outputs):
                extracted = [extract_code(output, self.model.model_style) for output in output_list]
                if not any(code.strip() for code in extracted):
                    retry_prompts.append(format_prompt_generation_code_only(benchmark[idx], self.model.model_style))
                    retry_indices.append(idx)
            if retry_prompts:
                print(f"Retrying {len(retry_prompts)} codegeneration prompts with code-only fallback")
                retry_outputs = self.prompts_to_outputs(retry_prompts)
                for idx, retry_output in zip(retry_indices, retry_outputs):
                    retry_extracted = [extract_code(output, self.model.model_style) for output in retry_output]
                    if any(code.strip() for code in retry_extracted):
