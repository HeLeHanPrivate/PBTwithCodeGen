...
class PromptConstants:
    ...
    SYSTEM_MESSAGE_DEEPSEEK_CODER_INPUT_GENERATOR = (
        ...
        "that dynamically creates valid counterexample-oriented input strings for a given programming problem description and platform. "
        "The generated script should use randomization seeded by current time, bias toward edge cases that expose bugs, "
        "and print exactly one string matching the required input format of the platform."
    )
...
def _get_common_dynamic_input_generation_user_content(
    question: str,
    example_input_str: str,
    platform: Platform,
    original_code_snippet: str,
    model_specific_output_format_instruction: str,
    repair_feedback: str = "",
) -> str:
    """
    Helper function to create the common part of the user prompt for dynamic input generation.
    """
    user_content = "You are an expert Python programmer specializing in generating valid, bug-finding problem inputs for competitive programming platforms using randomization. "
    ...
    user_content += "1. Generate only valid inputs according to the problem description and platform format.\n"
    ...
    user_content += "4. Bias the generated inputs toward cases likely to distinguish an incorrect solution from a correct one: minimum and maximum-small sizes, duplicate values, ties, all-equal data, alternating patterns, negative/zero values when allowed, sorted/reverse-sorted data, disconnected or degenerate structures, and off-by-one boundaries.\n"
    user_content += "5. Prefer many small or medium cases over one huge case. Small cases are important because independent property checkers may use brute-force oracles only under guarded tiny constraints.\n"
    user_content += "6. Deliberately generate inputs that can activate small exact oracles: tiny arrays/strings/graphs, small value ranges, few queries, few nodes, short paths, and exhaustive-search-sized instances.\n"
    user_content += "7. When the existing code already appears to handle examples, include harder valid probes: largest safe small cases for oracle checks, repeated values, ties, dense/sparse extremes, overlapping ranges, disconnected structures, and adversarial ordering.\n"
    user_content += "8. If the likely failure is timeout or hidden-only behavior, occasionally generate a larger but still safe stress case within the stated constraints, while keeping the script itself fast and deterministic enough not to time out.\n"
    user_content += "9. Use the original code only to infer parsing shape and suspicious branches; do not copy its assumptions when they conflict with the problem statement or examples.\n\n"
    ...
    input_generation_logic_guidance = (
        "First identify the exact input grammar and constraints. Then randomly generate values within those constraints and format them into the required string output. "
        "The generator should mix boundary cases with random cases: tiny sizes for oracle/checker feedback, largest guarded tiny cases for exact property checks, typical sizes for branch coverage, and occasional larger stress cases when constraints allow. "
        "A good generator should often produce inputs that an independent small brute-force checker can fully verify. "
        "For hidden or TLE-prone failures, prefer structured stress inputs over purely random ones, such as many equal values, many ties for an optimum, sorted and reverse-sorted orders, dense graphs versus sparse graphs, repeated substrings, overlapping intervals, or maximum-small query sets. "
        "When the format has multiple test cases, generate a valid test-count line and include several varied cases in one printed input.\n"
        "Every random bound must be safe. Before calling random.randint(a, b), random.randrange(a, b), random.sample, or choosing an index range, ensure the range is non-empty. "
        "If constraints make a random range uncertain, fall back to a fixed tiny valid case instead of raising an exception. "
        "Prefer helper functions such as safe_randint(a, b, default) that return the default when a > b.\n"
    )
    ...
    user_content += "The script may use helper functions, but its stdout must contain only the generated input string. Do not print debug text, explanations, expected outputs, or checker code.\n"
    user_content += "The script must never crash during repeated executions. Guard all random ranges, list indexes, graph edge counts, and sample sizes before using them.\n"
    user_content += "Before finalizing the script, mentally execute every helper once: no reference to a grid row before it is appended, no out-of-range mirror indexes, no unparenthesized generator expression passed with keyword arguments, and no random.sample request larger than the population.\n"
    user_content += "Include small safety helpers when useful, for example `safe_randint(a, b, default)`, `safe_choice(seq, default)`, and guarded `random.sample(pop, min(k, len(pop)))`. Define every helper before it is called, and call the exact function name you define.\n"
    user_content += "At the bottom, wrap generation in a simple main path that prints exactly once. Avoid hidden alternate entry points such as calling `generate_adversarial()` when only `generate_adversarial_case()` exists.\n"
    user_content += "If uncertain about the full constraints, choose conservative valid values that match the example grammar instead of inventing an incompatible format.\n"
    if repair_feedback:
        user_content += "\n### Previous Generator Failure To Fix\n"
        user_content += repair_feedback.strip()[:2500] + "\n"
        user_content += (
            "Rewrite the generator script to avoid this failure. Do not repeat the same "
            "undefined name, empty random range, uninitialized variable, timeout-prone loop, "
            "or out-of-range index pattern.\n"
        )
    user_content += model_specific_output_format_instruction # Add model-specific formatting here
    return user_content


def get_deepseek_r1_dynamic_input_generation_prompt(question: str, example_input_str: str, platform: Platform, original_code_snippet: str, repair_feedback: str = "") -> str:
    output_format_instruction = (
        "Remember to place your reasoning within `<think></think>` tags and the final Python script "
        "within `<answer>\n```python\n# YOUR PYTHON SCRIPT HERE\n```\n</answer>` tags."
    )
    user_content = _get_common_dynamic_input_generation_user_content(question, example_input_str, platform, original_code_snippet, output_format_instruction, repair_feedback)
...
def get_deepseek_coder_dynamic_input_generation_prompt(question: str, example_input_str: str, platform: Platform, original_code_snippet: str, repair_feedback: str = "") -> str:
...
def get_codeqwen_instruct_dynamic_input_generation_prompt(question: str, example_input_str: str, platform: Platform, original_code_snippet: str, repair_feedback: str = "") -> str:
...
def execute_inputer_script(script_code: str, num_executions: int = 5, timeout_seconds: int = 1, return_diagnostics: bool = False):
    generated_inputs = []
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if output:
                    ...
                    message = f"Warning: Inputer script execution {i+1} produced no output."
                    diagnostics.append(message)
                    print(message)
            else:
                message = (
                    f"Error: Inputer script execution {i+1} failed with return code {process.returncode}.\n"
            ...
            message = f"Error: Inputer script execution {i+1} timed out after {timeout_seconds} seconds."
            diagnostics.append(message)
            print(message)
            all_executions_successful = False
            break
        except Exception as e:
            message = f"Error: An unexpected error occurred during inputer script execution {i+1}: {e}"
            diagnostics.append(message)
            print(message)
            all_executions_successful = False
            break
...
def format_prompt_inputer_generate(
    ...
    repair_feedback = ""
    if len(platform_and_samples) >= 3:
        platform, samples_dict, repair_feedback = platform_and_samples[:3]
    else:
    ...
    try:
        ...
        input_output = samples_dict.get("input_output", {})
        if isinstance(input_output, str):
            input_output = json.loads(input_output)
        raw_inputs_data = input_output.get("inputs")
    ...
    if LanguageModelStyle == LMStyle.DeepSeekR1:
        user_content = get_deepseek_r1_dynamic_input_generation_prompt(question, example_input_str, platform, code, repair_feedback)
        ...
        user_content = get_deepseek_coder_dynamic_input_generation_prompt(question, example_input_str, platform, code, repair_feedback)
        ...
        user_and_assistant_part = get_codeqwen_instruct_dynamic_input_generation_prompt(question, example_input_str, platform, code, repair_feedback)
        ...
        return prompt
    elif LanguageModelStyle in [LMStyle.LocalAPI, LMStyle.DeepSeekAPI]:
        user_content = _get_common_dynamic_input_generation_user_content(
            question,
            example_input_str,
            platform,
            code,
            "\nReturn exactly one fenced Python code block containing the generator script and no prose outside it.\n```python\n# generator script here\n```\n",
            repair_feedback,
        )
        return [
            {
                "role": "system",
                "content": PromptConstants.SYSTEM_MESSAGE_DEEPSEEK_CODER_INPUT_GENERATOR,
            },
