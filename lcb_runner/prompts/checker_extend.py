...
class PromptConstants:
    ...
    SYSTEM_MESSAGE_DEEPSEEK_CODER_CHECKER = (
        ...
        "if properties hold, or throw AssertionError if a property is violated. "
        "Prefer small independent oracles and short assertion messages over broad rewrites."
    )
...
def get_metadata(put_run_exec, worker_id, samples, output_code, timeout, default_feedback):
    ...
    if not (bool(curr_res) and all(result is True for result in curr_res)):
...
def get_deepseek_coder_question_template_answer(question: str, code: str, result, metadata):
    ...
    user_prompt_content = (
        ...
        "Do NOT modify the original problem-solving logic or fix any bugs. Focus solely on adding verification code. "
        "Think about what properties the output (and intermediate states, if relevant) should satisfy based on the problem description.\n\n"
        "PGS-style checker rules:\n"
        "- Preserve the original public interface and stdin/stdout behavior exactly.\n"
        "- Add the smallest useful verification needed to produce semantic feedback; do not rewrite the solution.\n"
        "- Prefer output postconditions, conservation laws, inverse checks, and tiny guarded brute-force/differential oracles.\n"
        "- Public examples may guide semantics, but do not branch on exact sample constants or expected outputs.\n"
        "- Assertion messages should be short and name the violated property.\n"
        "- If the code is contest-style stdin/stdout, keep all original output behavior when checks pass and raise before printing only when a real inconsistency is detected.\n"
        "- If no reliable independent property can be added, return the original program unchanged.\n\n"
        "For example:\n"
        "### Problem Description Example:\n"
        "Write a function `PrimeFactorization(n)` that returns a list of prime factors of `n`.\n\n"
        "### Example Original Code:\n"
...
def format_prompt_checker_extend(
    ...
    if result:
        ...
        return ""
    if LanguageModelStyle == LMStyle.LocalAPI:
        return [
            {
                "role": "system",
                "content": PromptConstants.SYSTEM_MESSAGE_DEEPSEEK_CODER_CHECKER,
            },
            {
                "role": "user",
                "content": get_deepseek_coder_question_template_answer(
