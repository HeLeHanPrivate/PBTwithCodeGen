...
class PromptConstants:
    ...
    SYSTEM_MESSAGE_DEEPSEEK_R1 = (
        ...
        "The user asks a programming problem, and the Assistant writes the Python solution. "
        "The Assistant must keep reasoning brief and provide the complete solution immediately in one fenced Python code block. "
        "Do not spend the response on extended analysis; the first code block must contain the final answer.<｜User｜>"
    )
...
def get_problem_guidance(question: CodeGenerationProblem, max_public_tests: int = 5) -> str:
    guidance = []
    func_name = question.metadata.get("func_name") if isinstance(question.metadata, dict) else None
    if func_name:
        guidance.append(
            f"You must define the target function named `{func_name}`. "
            "The evaluator will call exactly this function name."
        )
        if question.starter_code and func_name not in question.starter_code:
            guidance.append(
                "The starter code may define a helper or a related function with a different name; "
                f"your final answer still must define `{func_name}`."
            )
    public_tests = getattr(question, "public_test_cases", []) or []
    if public_tests:
        guidance.append("Public tests/examples:")
        for test in public_tests[:max_public_tests]:
            if func_name:
                guidance.append(f"- {test.input}")
            else:
                guidance.append(f"- input: {test.input!r} output: {test.output!r}")
    if not guidance:
        return ""
    return "### Important Interface and Examples\n" + "\n".join(guidance) + "\n\n"


def get_generic_question_template_answer(question: CodeGenerationProblem):
    prompt = f"### Question:\n{question.question_content}\n\n"
    prompt += get_problem_guidance(question)
...
def get_deepseek_r1_question_template_answer(question: CodeGenerationProblem):
    ...
    if question.starter_code:
        ...
        prompt += f"{PromptConstants.FORMATTING_WITHOUT_STARTER_CODE}\n"
        prompt += f"```python\n# YOUR CODE HERE\n```\n\n"
    prompt += "Answer now with exactly one complete Python solution in one fenced code block.\n"
    prompt += f"<｜Assistant｜>"
    return prompt
...
def format_prompt_generation_code_only(question: CodeGenerationProblem, _language_model_style: LMStyle) -> str:
    prompt = (
        "Your previous response did not contain an extractable Python solution.\n"
        "Return only the complete Python solution in exactly one fenced code block.\n"
        "Do not include analysis, explanation, examples, or prose before the code.\n\n"
    )
    prompt += f"### Question\n{question.question_content}\n\n"
    prompt += get_problem_guidance(question)
    if question.starter_code:
